...
void sendPhotoToChat() {
  ...
  // 2. Отправляем команду для анализа
  delay(500); // Небольшая задержка между сообщениями
  bot.sendMessage(chat_id, "analyze_photo", "");
  
  bot.sendPhoto(chat_id, "image/jpeg", "Фото с камеры ESP32");
  Serial.println("Photo + analyze request sent");
}
...
const unsigned long PHOTO_INTERVAL = 60000; // 60 секунд
...
void loop() {
  ...
  if (millis() - lastPhotoTime >= PHOTO_INTERVAL) {
    sendPhotoToChat(); // Ваша функция для отправки фото
